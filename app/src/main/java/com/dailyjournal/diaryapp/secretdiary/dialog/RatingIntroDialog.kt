...
class RatingIntroDialog(activity: Activity, private val life: LifecycleOwner,
                        private val isFinishActivity: Boolean, private var action: () -> Unit) :
        ...
        try {
            if (SharePrefRemote.get_config(
                ...
                nativeBuilder.setListIdAd(AdmobApi.getInstance().getListIDByName("native_popup"))
                    val nativeManager = NativeManager(activity, life, nativeBuilder).setIntervalReloadNative( Contants.interval_reload_native)
            } else {
